    ...
    <el-row :gutter="10" class="mb8">
      <el-col :span="1.5">
        <el-button
          ...
          size="mini"
          @click="handleAdd"
          v-hasPermi="['amount:unsettled:add']"
        >新增</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          :disabled="single"
          @click="handleUpdate"
          v-hasPermi="['amount:unsettled:edit']"
        >修改</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          :disabled="multiple"
          @click="handleDelete"
          v-hasPermi="['amount:unsettled:remove']"
        >删除</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
...
<script>
import { listUnsettled, getUnsettled, delUnsettled, addUnsettled, updateUnsettled, exportUnsettled } from "@/api/amount/unsettled";
...
export default {
  ...
  methods: {
    ...
    getList() {
      this.loading = true;
      listUnsettled(this.queryParams).then(response => {
        this.unsettledList = response.rows;
      ...
      this.$refs["form"].validate(valid => {
        if (valid) {
          ...
          if (this.form.id != null) {
            ...
            addUnsettled(this.form).then(response => {
              if (response.code === 200) {
    ...
    handleDelete(row) {
      ...
      this.$confirm('是否确认删除已发货未结算金额编号为"' + ids + '"的数据项?', "警告", {
          confirmButtonText: "确定",
          cancelButtonText: "取消",
          type: "warning"
        }).then(function() {
          return delUnsettled(ids);
        }).then(() => {
          this.getList();
    ...
    handleExport() {
      ...
      this.$confirm('是否确认导出所有已发货未结算金额数据项?', "警告", {
          confirmButtonText: "确定",
          cancelButtonText: "取消",
          type: "warning"
        }).then(function() {
          return exportUnsettled(queryParams);
        }).then(response => {
